...
{
    public partial class EventBreakerViewModel : DeviceBaseVm
    {
        #region Properties
        ...
        [SetsRequiredMembers]
        public EventBreakerViewModel(string stationId, string id)
        {
            Id = id;
            StationId = stationId;
            _popupService = AppContainer.ServiceProvider.GetRequiredKeyedService<IPopupService>(Consts.Ioc_Device);
            CircuitEventManager.SubscribeAsync(OnDataReceivedAsync);
        }
        ...
        private async ValueTask OnDataReceivedAsync(object? sender, CircuitEventArgs args)
        {
        ...
        public void Dispose()
        {
            CircuitEventManager.UnSubscribeAsync(OnDataReceivedAsync);
        }
