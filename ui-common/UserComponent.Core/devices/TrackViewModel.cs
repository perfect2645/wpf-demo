...
{
    public partial class TrackViewModel : DeviceBaseVm
    {
        #region Properties
        ...
        [SetsRequiredMembers]
        public TrackViewModel(string stationId, string id)
        {
            Id = id;
            StationId = stationId;
        }
        ...
        private async void OnChannelDataReceivedAsync()
        {
            ...
            try
            {
                ...
                {
                    Log4Logger.Logger.Debug($"Update track message handling:[{data.StationId},{data.Id},{data.PowerOn}].");
